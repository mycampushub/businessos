...
export async function POST(req: NextRequest) {
  try {
    // H1 fix: rate-limit account creation — 5 signups/hour/IP to prevent spam + scrypt-DoS.
    const ip = clientIp(req)
    const ipLimit = checkRate(`register:${ip}`, 5, 60 * 60_000)
    if (!ipLimit.allowed) {
      return NextResponse.json(
        { ok: false, error: 'Too many signups from this address. Please try again later.' },
    ...
    // C16 fix: auto-verify email in sandbox (no SMTP) so new users can use the app immediately.
    // In production, remove emailVerified and keep emailVerifyToken for real email delivery.
    const emailVerifyToken = randomUUID()
    const user = await db.user.create({
      data: { name, email, passwordHash: hashPassword(password), emailVerifyToken, emailVerified: new Date() },
      select: {
