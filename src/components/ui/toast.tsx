...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // L24-ui: always bottom — full-width bar on mobile (covering the sticky
      // header was the bug), pinned bottom-right with a 420px max on sm+. The
      // slide-in animation is bottom-anchored on every breakpoint so toasts
      // emerge from the same edge they sit on.
      "fixed bottom-0 left-0 right-0 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-0 sm:right-0 sm:left-auto sm:w-auto sm:flex-col md:max-w-[420px]",
      className
    )}
    {...props}
  />
